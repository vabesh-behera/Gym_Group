...
import { REGION_NAMES, CLUB_SEED, MECHANICS_SEED, OBJECTIVE_LABELS } from "../lib/constants";
import { simulateCampaign, AVG_MONTHLY_FEE_GBP } from "../lib/simulate/elasticity";
import { seededRandom, seededRange } from "../lib/rand";

const DEMO_PASSWORD = "GymGroup2026!";
...
async function main() {
  console.log("Seeding GymIQ demo data…");

  // ---- User ----
  const passwordHash = await bcrypt.hash(DEMO_PASSWORD, 10);
  const user = await prisma.user.upsert({
    where: { email: "sarah.johnson@gymgroup.co.uk" },
    update: {},
    create: {
      email: "sarah.johnson@gymgroup.co.uk",
      name: "Sarah Johnson",
  ...
  console.log("Seeding club utilisation…");
  ...
  console.log("Seeding portfolio monthly metrics…");
  ...
  console.log("Seeding member events…");
  ...
  // ---- Change log ----
  console.log("Seeding change log…");
